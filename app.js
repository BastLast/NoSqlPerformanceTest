...
app.post('/RequestSelect', async (req, res) => {
    if (mode === "MySQL")
        ...
        const query = `MATCH (n:`+ req.body.table + `) RETURN n LIMIT 25`;
        const params = {};
...
app.post('/ProductListFollowers', async (req, res) => {
    if (mode === "MySQL") {
        ...
        const query = `MATCH (p1) WHERE id(p1) = ` + req.body.userId + ` MATCH path = (p1)<-[:FOLLOW*1..` + req.body.niveau + `]-(p:Person) WITH distinct p as p MATCH (p)-[:BUY]-(r:Product) WITH count(r) as nbrProduct, r RETURN r.refProd as firstName, nbrProduct as total;`;
        const params = {};
        await nosqlManager.requestNoSQL(query, params, res);
    }
})
...
app.post('/ProductListFollowersWithSpecificProduct', async (req, res) => {
    if (mode === "MySQL") {
        ...
        const query = `MATCH (p1) WHERE id(p1) = ` + req.body.userId + ` MATCH path = (p1)<-[:FOLLOW*1..` + req.body.niveau + `]-(p:Person) WITH distinct p as p MATCH (p)-[:BUY]-(r:Product) WHERE id(r) = "`+ req.body.productId +`" WITH count(r) as nbrProduct, r RETURN r.refProd as firstName, nbrProduct as total;`;
        const params = {};
        await nosqlManager.requestNoSQL(query, params, res);
    }
})
...
app.post('/CommandListByInfluence', async (req, res) => {
    if (mode === "MySQL") {
        ...
        const query = `MATCH (p:Person)-[:BUY]->(ref:Product) WHERE id(ref) = ` + req.body.productId + ` WITH distinct(p) as p CALL apoc.neighbors.athop(p, "<FOLLOW",` + req.body.niveau + `) YIELD node as result MATCH (result)-[:BUY]->(ref:Product) WHERE id(ref)=` + req.body.productId + `RETURN count(distinct(result));`;
        const params = {};
        await nosqlManager.requestNoSQL(query, params, res);
    }
})
